...
for linea in pos.readlines():
	if linea[0] == ">":
		...
		sres=open(sys.argv[1]+"/OutPutFiles"+sys.argv[2]+"/Modeller/"+linea[1]+linea[2]+linea[3]+linea[4]+linea[5]+linea[6]+".pdb.done/FrustrationData/"+linea[1]+linea[2]+linea[3]+linea[4]+linea[5]+linea[6]+".pdb_singleresidue",'r')
		sres.readline()
	else:
		ter=0
		slinea=linea.split(" ")
