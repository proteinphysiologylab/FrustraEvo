...
################################################################################# ADDED BY MFERNAN3 TO AUTOMATICALLY MATCH THE SEQS ################################################################################################
                ...
                flag_seqs = 1 #flag = 1 when at least one seq missmatches
    ...
    if flag_seqs == 1:
        clean_file=path_direc + '/Seqs_Clean.fasta'
        #realign with mafft
        mafft_cline = MafftCommandline(input=clean_file, maxiterate=1000, auto=True, quiet=False)
        stdout, stderr = mafft_cline()
        #re-write the MSA_Clean.fasta with the new alignment
        output_file = path_direc + '/MSA_Clean.fasta'
        
        # Write the MAFFT output to a temporary file
        temp_msa_file = path_direc + '/temp_MSA_Clean.fasta'
        with open(temp_msa_file, "w") as temp_out_file:
            temp_out_file.write(stdout)
               
        # Remove line breaks from sequences and save to the final output file
        remove_line_breaks(temp_msa_file, output_file)
        print('Multi-fasta file with retrieved sequences re-aligned with --auto parameters in mafft')

        os.system(f"cp {output_file} {path_direc + '/MSA_Clean_aux.fasta'}")
        print('MSA_Clean_aux.fasta file subsituted with re-aligned sequences')

def remove_line_breaks(input_file, output_file):
    """Remove line breaks from sequences in a FASTA file. 
    ADDED BY MFERNAN3 TO REFORMAT THE MAFFT OUTPUT"""

    with open(input_file, "r") as in_file, open(output_file, "w") as out_file:
        sequence = ""
        for line in in_file:
            if line.startswith(">"):  # If it's a header line (sequence ID)
                if sequence:  # If there's a sequence collected, write it before the next header
                    out_file.write(sequence + "\n")  # Write the previous sequence on a new line
                    sequence = ""  # Reset sequence for the next record
                out_file.write(line)  # Write the header (sequence ID)
            else:
                # Add the sequence part to the current sequence without line breaks
                sequence += line.strip()
