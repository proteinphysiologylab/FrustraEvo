...
msa = read.fasta(file = "./SalidaAlignSE.fasta",
                 forceDNAtolower = F,
                 seqtype = "AA")

FileTabla2 = read.table(file = "./CharactPosDataN",
                        header = T,
                        stringsAsFactors = F)
...
if (N > 100) {
  df_plot = data.table(
    ...
    state = rep(c('Minimum', "Maximum", "Neutral"), each =
                  N),
    group = c(rep(1, 99), rep(
      2:100, each = 100,
      length.out = ifelse(N > 99, N -
                            99, N)
    ))
  )
...
for (i in 1:max(df_plot$group)) {
  ...
  bplot = ggplot(subset(df_plot, group == i), aes(x = N, y = h, fill = state)) +
    ...
    scale_x_continuous(breaks = c(2, seq(10, 1998, by = 10))) +
    theme_logo() +
    xlab("") +
    ylab("") +
  ...
  list_plots[[i]] = plot_grid(
    slog + theme(legend.position = "none"),
...
svg("figureS9.svg", height = 4 * scale, width = 20)
plot_grid(
