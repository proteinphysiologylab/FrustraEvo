...
salida=Sys.glob(file.path(output, "OutPutFiles/"))
##########
### CODE##
##########
...
#  rs = pivot_wider(toreshape, names_from = Alignment_position, values_from = FrstIndex)
  ...
  # Clasificar los valores de FrstIndex en categorías utilizando cut()
  categorias <- cut(mapping_file$FrstIndex, 
                  breaks = c(-Inf, -1, 0.55, Inf), 
                  labels = c("Red", "Grey", "Green"))

  # Actualizar el código de ggplot con los colores asignados a cada categoría
  g1 <- ggplot(mapping_file, aes(x = Alignment_position, y = id, fill = categorias)) +
  geom_tile(linejoin = "round", width = 0.96, height = 0.96, color = 'grey80') +
  geom_text(aes(label = AA_seq), size = 12) +
  scale_fill_manual(name = "Single Residue FI",
                    values = c("Red" = "red", "Grey" = "grey", "Green" = "green"),
                    labels = c("Max", "Neu", "Min"),
                    na.value = "white") +
  theme_minimal() +
  ylab(NULL) +
  xlab(NULL) +
    theme(legend.position = "top",
          text = element_text(size = 30),  # Aumentar el tamaño de la letra en general a 12 (ajusta según tus necesidades)
          plot.title = element_text(size = 30),  # Aumentar el tamaño del título del gráfico a 14 (ajusta según tus necesidades)
          axis.text = element_text(size = 50),  # Aumentar el tamaño del texto de los ejes a 10 (ajusta según tus necesidades)
...
svg(paste(salida, "MSFA.svg", sep=""),pointsize = "100",width = 100, height = 30)
p
invisible(dev.off())
library(magick)
testimage <- image_read_svg(paste(salida, "MSFA.svg", sep=""), width = 2500)
image_write(testimage, path = paste(salida, "MSFA.png", sep=""), format = "png")
